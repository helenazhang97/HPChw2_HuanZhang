...
#define ib  BLOCK_SIZE
#define kb  BLOCK_SIZE
...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  // TODO: See instructions below
	...
	for (ii=0;ii<m;ii+=ib)//stride for loading a by row stripe (ib x k)
	{
			for (kk=0;kk<k;kk+=kb)//multiply length kb subvectors 
			{
				for (j=0;j<n;j+=2)//do 2 colomns of B together
				{
						for(i=ii;i<ii+ib;i+=2)//do 2 rows of a stripe together
						{
								if (kk==0)
								...
								for (p=kk;p<kk+kb;p++)
								{
									acc00+=b[p+(j+0)*k]*a[i+0+(p*m)];
									acc01+=b[p+(j+1)*k]*a[i+0+(p*m)];
									acc10+=b[p+(j+0)*k]*a[i+1+(p*m)];						
									acc11+=b[p+(j+1)*k]*a[i+1+(p*m)];						
								}
								c[i+0+(j+0)*m]=acc00;
								c[i+0+(j+1)*m]=acc01;
								c[i+1+(j+0)*m]=acc10;
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = 2*k*m*n*NREPEATS/time*1e-9; // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = sizeof(double)*(m*n*(2*k/kb)+(m/ib)*n*k+k*m)*NREPEATS/time*1e-9; // TODO: calculate from m, n, k, NREPEATS, time
